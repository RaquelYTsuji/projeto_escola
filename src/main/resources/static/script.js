...
cursoForm.addEventListener("submit", async (e) => {
  ...
  try {
    if (editingCourseId) {
      await api(`/cursos/${editingCourseId}`, {
        method: "PUT",
...
usuarioForm.addEventListener("submit", async (e) => {
  ...
  try {
    if (tipo === "ALUNO") {
      ...
      const curso = userCurso.value;
      if (!turma || !curso) {
        alert("Para Aluno: informe Turma e selecione um Curso.");
        return;
      }
      const dto = { id: editingUser.id ?? null, nome, cpf, curso, turma };

      if (editingUser.id && editingUser.type === "ALUNO") {
        await api(`/alunos/${editingUser.id}`, { method: "PUT", body: JSON.stringify(dto) });
      } else {
        await api("/alunos", { method: "POST", body: JSON.stringify(dto) });
      }
      await listarAlunos();
      ...
      if (editingUser.id && editingUser.type === "PROFESSOR") {
        await api(`/professores/${editingUser.id}`, { method: "PUT", body: JSON.stringify(dto) });
      } else {
        await api("/professores", { method: "POST", body: JSON.stringify(dto) });
      }
...
window.editarAluno = async function(id) {
  try {
    ...
    if (a.curso) userCurso.value = a.curso;
...
window.editarProfessor = async function(id) {
  try {
    const p = await api(`/professores/${id}`);
